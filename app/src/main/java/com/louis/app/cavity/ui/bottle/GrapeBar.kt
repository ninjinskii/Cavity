...
class GrapeBar @JvmOverloads constructor(
    ...
    fun addAllGrapes(grapes: List<Grape>) {
        grapes.forEach { prepareBar(it) }
    }

    fun triggerAnimation() {
        bars.forEach {
            ObjectAnimator.ofInt(it.first, "progress", 0, it.second * 10).apply {
                duration = 800
                start()
            }
        }
    ...
    private fun prepareBar(grape: Grape) {
        ...
        progressBar.apply {
            ...
            progressTintList = ColorStateList.valueOf(colors[childCount])
        }
